...
import { YthShopFormService } from 'src/app/services/yth-shop-form.service';
import { YthShopValidators } from 'src/app/validators/yth-shop-validators';
...
export class CheckoutComponent implements OnInit {
  ...
  stripe_api = environment['stripePublishableKey'];
  stripe = Stripe(this.stripe_api);
  ...
  ngOnInit(): void {
    ...
    this.checkoutFormGroup = this.formBuilder.group({
      customer: this.formBuilder.group({
        firstName: new FormControl('', [
          ...
          YthShopValidators.notOnlyWhitespace,
        ]),
    ...
    // populate states
    // this.luv2ShopFormService.getStates().subscribe((data) => {
  ...
  setupStripePaymentForm() {
    ...
    // add an sintance of card UI component into the 'card-element' div
    this.cardElement.mount('#card-element');
    // add event binding for the change event on the card element
    this.cardElement.on('change', (event: any) => {
      // get a hanle to card-errors element
  ...
  reviewCartDetails() {
    // subscribe to cartService.totalQuanitty
  ...
  onSubmit() {
    console.log('Handling the submit process');
        ...
        (paymentIntentResponse) => {
          ...
          .then((result: any) => {
            if (result.error) {
              ...
              alert(`There was an fucking error: ${result.error.message}`);
